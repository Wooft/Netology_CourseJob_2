...
def get_user_and_persons_info_from_vk(user_id, token, offset):
    ...
    if fitted_person[0]['is_closed'] == True:
        offset += 1
        get_user_and_persons_info_from_vk(user_id, token, offset)
    else:
        pass
        #Проверка на то, что у пользователя есть хотя бы 3 фото в профиле
...
token = 'vk1.a.CDg6S4Z-DWuUEXId3MDPW8fOUlxpq1G_7JJZzK1w8xYMMa4Qmnbxz3WmvIefUPw6vNPFom3UfZ-C08QVa_Xql7GSRwmAEdIVz_TOqnWFji1LhVkR4tLl7tY08XFk8bUdO3KtfpXtYGn3FiavD4PY9ADOnCTsNbCNn7u0il19D7adMVi1VgCjdZqylFPZ5AoF'
# get_user_and_persons_info_from_vk(user_id=15565301, token=token, offset=1)
get_user_photo(user_id=15565301, token=token)